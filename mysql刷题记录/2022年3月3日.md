...
SELECT
	machine_id,
	ROUND( sum( IF ( activity_type = 'end', TIMESTAMP,- TIMESTAMP )) / count( DISTINCT process_id ), 3 )
          	AS processing_time 
FROM
	activity 
GROUP BY
...
ROUND(
	(
		sum(IF( activity_type = 'end', TIMESTAMP, 0 ))- sum(IF( activity_type = 'start', TIMESTAMP, 0 ))
